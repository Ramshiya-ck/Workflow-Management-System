...
/**
 * Visual banner showing query reset parameters for empty billing search lists.
 */
const BillEmptyState = ({ onReset, hasFilters }) => {
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-white border border-zinc-200/80 rounded-xl text-center max-w-sm mx-auto my-6 font-sans select-none">
      ...
      <h3 className="text-sm font-bold text-zinc-900">No Bills Found</h3>
      <p className="text-xs text-zinc-450 mt-1 leading-relaxed">
        {hasFilters
          ? "No registered invoices match the selected filters, vendor constraints, or date ranges."
          : "There are no bill records registered in the system workflow lists yet."}
      </p>
      {hasFilters && (
...
export default React.memo(BillEmptyState);
