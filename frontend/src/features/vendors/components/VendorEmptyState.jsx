...
/**
 * Visual banner showing query reset parameters for empty supplier outputs.
 */
const VendorEmptyState = ({ onReset, hasFilters }) => {
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-white border border-zinc-200/80 rounded-xl text-center max-w-sm mx-auto my-6 font-sans select-none">
      ...
      <h3 className="text-sm font-bold text-zinc-900">No Vendors Found</h3>
      <p className="text-xs text-zinc-450 mt-1 leading-relaxed">
        {hasFilters
          ? "No registered suppliers match the selected search criteria or status filter parameters."
          : "There are no vendor profiles registered in the system index registries yet."}
      </p>
      {hasFilters && (
...
export default React.memo(VendorEmptyState);
